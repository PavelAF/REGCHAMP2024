...
if [[ "$switch" == 2 ]]; then
	...
	until read -p $'Действие: 1 - активировать аккаунты, 2 - отключить аккаунты, 3 - установить пароли\nВыберите действие: ' switch3; [[ "$switch3" =~ ^[1-3]$ ]]; do true;done
	
	list=''
	for ((stand=$switch; stand<=$switch2; stand++))
	{
		[ $switch3 == 1 ] && pvesh set access/users --userid $comp_name$stand@pve --enable 1
		[ $switch3 == 2 ] && pvesh set access/users --userid $comp_name$stand@pve --enable 0
		[ $switch3 == 3 ] && \
		( 
...
until read -p $'Ввведите начальный идентификатор ВМ: ' switch; [[ "$switch" =~ ^[1-9][0-9]*$ ]] && [[ "$switch" -lt 900000 && "$switch" -ge 100 ]]; do true;done
start_vm_id=$switch

until read -p $'Ввведите начальный идентификатор vmbr бриджей: ' switch; [[ "$switch" =~ ^[1-9][0-9]*$ ]] && [[ "$switch" -lt 900000 && "$switch" -ge 100 ]]; do true;done
start_vmbr_id=$switch
...
for ((stand=$switch; stand<=$switch2; stand++))
{
	...
	pvesh create access/users --userid $comp_name$stand@pve --comment 'Учетная запись участника соревнований'
...
IFACE
	...
	qm create $bame_vm_id --name "ISP" --cores 1 --memory 1024 --startup order=1,up=10,down=30 --net0 virtio,bridge=vmbr0 --net1 virtio,bridge=${Networking['ISP<=>RTR-HQ']} --net2 virtio,bridge=${Networking['ISP<=>RTR-BR']} --vga serial0 --serial0 socket --agent 1 --ostype l26 --scsihw virtio-scsi-single 
	qm importdisk $bame_vm_id ISP.vmdk $STORAGE --format qcow2 
	qm set $bame_vm_id --scsi0 $STORAGE:vm-100-disk-0 --boot order=scsi0
	echo "ISP is done!!!"

	qm create $((bame_vm_id+1)) --name "RTR-HQ" --cores 2 --memory 1536 --tags 'alt_server' --startup order=2,up=20,down=0 --net0 virtio,bridge=${Networking['ISP<=>RTR-HQ']} --net1 virtio,bridge=${Networking['RTR-HQ<=>SW-HQ']} --serial0 socket --acpi 0 --ostype l26 --scsihw virtio-scsi-single
	...
	qm create $((bame_vm_id+6)) --name "RTR-BR" --cores 2 --memory 1536 --tags 'alt_server' --startup order=2,up=20,down=0 --net0 virtio,bridge=${Networking['ISP<=>RTR-BR']} --net1 virtio,bridge=${Networking['RTR-BR<=>SW-BR']} --serial0 socket --acpi 0 --ostype l26 --scsihw virtio-scsi-single
	...
	pvesh set /pool/$stand_name$stand -vms "`seq -s, $bame_vm_id 1 $((bame_vm_id+9))`"
